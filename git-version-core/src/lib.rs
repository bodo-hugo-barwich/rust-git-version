...
//use git_version::git_version_macro::utils::{describe_cwd, git_dir_cwd};
...
pub fn try_git_version(input: TokenStream) -> TokenStream {
	let args = parse_macro_input!(input as Args);

	let tokens = /*match*/ git_version_impl(args).unwrap();
	/* {
	  Ok(x) => x,
...
pub fn git_version_impl(args: Args) -> syn::Result<TokenStream2> {
